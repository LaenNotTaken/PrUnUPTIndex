...
async def poptc(ctx):
    try:
        ...
        def calculate_averages(
            commodities_by_currency, total_sum_by_currency, count_by_commodity
        ):
            averages_by_currency = {}
            for currency, commodities in commodities_by_currency.items():
                total_commodities = sum(
                    1 for commodity in commodities if commodities[commodity] > 0
                )
                ...
                averages_by_currency[currency] = (
                    rank,
                    "{:.2f}".format(average_sum / total_commodities),
                )
        ...
        for company_code in preselected_companies:
            ...
            for order in exchange:
                ...
                for buy in order.Buys:
                    ...
                    commodities_by_currency_buys[currency][commodity] = (
                        commodities_by_currency_buys[currency].get(commodity, 0)
                        + total_buy_cost
                    )
                ...
                for sell in order.Sells:
                    ...
                    commodities_by_currency_sells[currency][commodity] = (
                        commodities_by_currency_sells[currency].get(commodity, 0)
                        + total_sell_cost
                    )
                    total_sells_by_currency[currency] += total_sell_cost

                count_by_commodity[commodity] = (
                    count_by_commodity.get(commodity, 0)
                    + sum(buy.Count for buy in order.Buys)
                    + sum(sell.Count for sell in order.Sells)
                )
            # Get averages for each commodity currency pair
            averages_buys = calculate_averages(
                commodities_by_currency_buys, total_buys_by_currency, count_by_commodity
            )
            averages_sells = calculate_averages(
                commodities_by_currency_sells,
                total_sells_by_currency,
                count_by_commodity,
            )
            ...
            for currency in set(averages_buys.keys()).union(averages_sells.keys()):
                ...
                buy_line = format_line("FMV", buy_rank, buy_avg, currency_abbr).ljust(
                    37
                )
                sell_line = format_line(
                    "CPV", sell_rank, sell_avg, currency_abbr
                ).ljust(37)
        ...
        if company_blocks:
            ...
            current_message = ""
            company_count = (
                0  # Initialize a counter for the number of "Company:" occurrences
            )
            ...
            for line in transposed_lines:
                ...
                if (
                    len(current_message) + len(line) <= message_limit
                    and company_count <= 3
                ):
                    ...
                    current_message = line + "\n"
                    company_count = (
                        1  # Reset the counter because a new message is started
                    )
...
async def on_ready():
    try:
        ...
        logging.exception("Error occurred in on_ready")
